...
RADIO_CURRENTS_3V_DCDC_0DBM_A = {
    'nrf52840': {'ISTART,TX,DCDC': 0.0052, 'ITX,0dBM,DCDC': 0.0048, 'ISTART,RX,1M,DCDC': 0.0037, 'IRX,2M,DCDC': 0.0052},
    'nrf5340':  {'ISTART,TX,DCDC': 0.0024, 'ITX,0dBM,DCDC': 0.0034, 'ISTART,RX,1M,DCDC': 0.0021, 'IRX,2M,DCDC': 0.0031}
}


def _on_air_len_2mbps_us(pl_len_bytes, pcf_bits):
    """ References:
    ...
    return float(preamble_bits + address_bits + pcf_bits + pl_len_bits + crc_bits) / 2
...
def _pkt_len_2mbps_us(pl_len_bytes, pcf_bits=11, ramp_up_us=40):
    ...
    """
    pkt_len_us = _on_air_len_2mbps_us(pl_len_bytes, pcf_bits)
...
def pkt_energy_tx_2mbps_j(pl_len_bytes, device_str, pcf_bits=11, ramp_up_us=40):
    ...
    tx_j = 3 * tx_a * _on_air_len_2mbps_us(pl_len_bytes, pcf_bits) / 1000000
    return (start_j + tx_j)
...
def pkt_energy_rx_2mbps_j(pl_len_bytes, device_str, pcf_bits=11, ramp_up_us=40):
    ...
    start_j = 3 * start_a * ramp_up_us / 1000000
    rx_j = 3 * rx_a * _on_air_len_2mbps_us(pl_len_bytes, pcf_bits) / 1000000
    return (start_j + rx_j)
